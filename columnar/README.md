...
In that case, `velocore-columnar`'s behavior is as follows:
...
from Number or boolean values. `velocore-columnar` will simply emit several columns associated to a given column_name.
...
`velocore-columnar` will pick the first type that can represents the set of appended value, with the following prioriy order (`i64`, `u64`, `f64`).
