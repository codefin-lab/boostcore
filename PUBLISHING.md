# Publishing VeloCore to crates.io
...
0.9.0, so `velocore-common` takes it from crates.io like anyone else.
...
They are tantivy's, because the code is: `velocore` is 0.26.1, and each
...
BoostSearch depends on VeloCore by path, at `vendor/velocore`. Once these are
